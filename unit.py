...
class Unit(pygame.sprite.Sprite):
	class Projectile(pygame.sprite.Sprite):
		...
		def __init__(self, rect, direction):
			...
			self.image = pygame.image.load(os.path.join("images", "projectile.png"))
			if direction == K_LEFT:
	...
	def fire(self, projectiles, collidable, grid, screen):
		...
		# create projectile
		projectile = self.Projectile(pygame.Rect(self.rect), self.direction)
		...
		if self.direction == K_RIGHT:
			top = top + 25
			right = right + 25
			while right < grid.map_width*grid.pix_width:
			...
			bottom = bottom + 25
			left = left + 25
			while bottom < grid.map_height*grid.pix_height:
				for item in collidable_objects:
